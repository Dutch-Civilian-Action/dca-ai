...
| Confirmed action has no agreed due date or fully specified definition of done | Ask the owner or authorised assigner for a date; capture the bounded action with one verified assignee and source/confirmation links. Leave the Asana date empty until a date or bounded window is known, and qualify the missing outcome rather than inventing it. |
| On 25 September in Europe/Amsterdam, an owner confirms work for "this weekend" (26–27 September) | Set the Asana date to Saturday 26 September and state 26–27 September in the task description; explain that the Asana date marks the window's start. |
| On 25 September in Europe/Amsterdam, an owner confirms a check-in for "next week" | Set the Asana date to Monday 28 September and state 28 September–2 October in the description. Keep dependencies and the absence of an exact appointment distinct. |
| A proposed task says only "soon" with no anchored timeframe | Ask for a date; leave the Asana date empty while awaiting one. Do not derive a calendar date from the vague wording or block otherwise confirmed bounded work. |
