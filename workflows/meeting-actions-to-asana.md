...
Include the source link and passage/timestamp for each item. State that confirmation approves the displayed task scope, assignment, destination and any date for capture in Asana; it does not approve executing the underlying action. Show a proposed destination where task creation is requested; if routing is unresolved, keep that item pending. If the due date is unknown, explicitly ask the proposed owner or authorised assigner for it in the review batch. Do not block an otherwise clear task while waiting for an exact date. Name only source-supported relevant participants and their roles, and show any proposed Asana collaborators for review separately from the assignee. Invite an authenticated participant to reply with the item reference and a request such as "Add me as a collaborator on H17-02" or "Tag me for the H17-02 review question". A collaborator request does not approve that item's task scope or ownership.
...
A request to follow a task is independent of assignment approval. Resolve the requester's Asana identity and access; add them as a collaborator only to the matching existing or newly confirmed task, within the task's visibility and authority boundary. A specific request to be tagged for an input or decision calls for a targeted mention on that task when the question is ready, not an automatic follower grant. Do not add all meeting attendees, infer collaborators from mere participation, or treat another person's suggestion as their opt-in. Preserve a pending request if the task or access is not yet available, and check current followers before retrying so the same request does not notify twice. For an anchored approximate period or timeframe, use its earliest date as the Asana due date and put the exact start–end dates in the description. For example, "next week" stated on Friday 25 September 2026 in Europe/Amsterdam maps to Monday 28 September as the Asana date and 28 September–2 October as the described window. Say that the Asana date marks the start of the window, not a promise of completion that day. Resolve relative dates using the meeting/reply's Europe/Amsterdam context and preserve conditions or uncertainty. If no date or bounded window can be established, ask for a date and leave the field empty while awaiting it; this alone does not block an otherwise clear task. Material uncertainty about ownership, scope, identity, destination or permission does block creation. Other unresolved facts may remain qualified if the confirmed task does not depend on treating them as settled.
...
- **Assignee:** exactly one verified person responsible for the confirmed action. Keep a precise agreed due date when one exists. For an anchored approximate window, set the first day as the Asana date and state the complete date range in the description. Ask for an unknown date and leave it empty until an exact date or bounded window is supplied. Do not invent a date to satisfy a template.
- **Description:** give a brief objective, the confirmed requirements/scope, relevant accessible resources, the full date window where applicable, and the expected result or definition of done if supported. Include the meeting source, stable item reference and passage/timestamp; the confirmation link, authoriser and time; and material dependencies, qualifications and known results. Mark an unknown outcome or dependency as such rather than writing an invented acceptance criterion.
...
| H17-02 | James tries Ukrainian-partner contact retrieval/storage, then follows up with Anja | 52:44–53:19; 1:32:08–1:33:01 | Confirm trial scope and intake readiness. Check overlap with H17-05; "next week" was mentioned by the shared connection, but the scheduling owner and applicable window were not yet confirmed. Ask the confirmed owner for the date/window before capture. |
