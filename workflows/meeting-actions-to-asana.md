...
Treat AI-generated meeting notes as a first draft, not the authoritative account of the meeting. Before deriving actions, check the existing notes against the available transcript/recording, meeting thread and participant corrections. Review attendance, attribution, summary, material omissions, decisions, action items, dependencies, timing and unresolved questions. This applies to meeting notes generally, including huddle canvases.

Before editing published notes, verify edit access for the identity that will actually perform the write. Channel membership, meeting attendance, source-read access or a human participant's own access does not prove the selected writer can edit them. Where the notes are a shared document or canvas, edit access must be granted to that writer identity specifically: ask the notes owner or an authorised editor to share them with that identity with edit rights, then recheck. Keep the notes update blocked until access and write/readback are verified; use the existing pending-patch fallback below. Do not grant access yourself unless authorised. No default automatic-sharing mechanism is established by this pilot; do not claim such sharing is impossible.
...
An owner replies in this thread with the stable item reference. Whether that reply reaches the executor depends on the selected runtime's **verified** invocation model: a runtime may watch or poll a registered thread, or it may act only when a message explicitly invokes it. Establish which applies — for the initial processing request and for each subsequent owner reply — before publishing the batch, and state the resulting processing mode and any required invocation step in the batch itself. Do not assume automatic detection, and do not tell owners that a plain reply suffices unless that has been observed for the selected runtime. Receipt in the meeting surface, detection by the executor, authorisation and verified Asana capture remain distinct. Sharing notes, adding the executor to a channel or invoking it once does not itself register a meeting or grant recurring processing.

One designated executor owns notes correction, the action review batch, item confirmations, Asana capture and returned mappings for a meeting. It must hold, under its own identity, the access each of those steps needs — source-thread read, notes edit, Asana write — and remain inside the runtime's existing action-authorisation rules. Its identity, invocation model, cadence, connectors, scope and current status belong in its provider implementation record: [Claude](../providers/claude/meeting-actions-to-asana.md), [ChatGPT](../providers/chatgpt/meeting-actions-to-asana.md). Retain authorised meeting registrations and pending items in their existing source threads and continue older pending batches; access alone does not register a new meeting.

The [validation monitor](monitor-validation-queue.md) checks validation of the workflow's test, not operational ownership or execution of this workflow. It may inspect outcomes as evidence, but must not edit these notes, collect action confirmations, dispatch capture, mutate operational tasks or send operational action reminders. A pending action is not itself a pending test-validation judgement. Keep test evidence and any exact test-review question linked to the existing pilot-validation lineage. Test validation is not an execution prerequisite for the already authorised bounded workflow, and does not block it.

If the designated executor cannot complete a capture, preserve the confirmed-but-blocked handoff in its own batch. Do not transfer execution to the validation queue or make Anja the default chaser. This workflow adds no automatic reminder stream; follow-up contact beyond its source-thread review requires its own authorisation.
...
If task creation succeeds but the source-thread update fails, retain the verified task ID and retry only the thread update. Do not claim successful handoff until the mapping is visible. Keep confirmed-but-unwritten or unverified items distinct from items still awaiting human confirmation.
...
Use [the acceptance cases](../tests/workflows/meeting-actions-to-asana.md). A supervised ChatGPT pilot on 18 September 2026 verified in-place notes correction and one review-batch publication. An initial transcript download failed; the access recheck recovered the attachment, completed review of the available transcript text, and verified further corrections to the same notes and review batch. Its evidence and limitations are recorded in the acceptance cases. Later evidence extends that initial result: H17-03 was reconciled as already done on 19 September; on 21 September H17-01 was confirmed already done, one H17-05/H17-02 scheduling task and one H17-06 guidance task were created and read back with source-thread mappings. H17-02's trial/follow-up and H17-04 remain pending. These are dated dispositions, not a rewrite of the original extraction. The earlier hourly validation-monitor configuration included operational pilot handoffs; Anja corrected that boundary on 21 September. The designated executor owns execution, while the validation queue tracks only test validation. General meeting ingestion is not enabled.

No executor is currently active for this workflow. The ChatGPT runner is paused (see its [provider record](../providers/chatgpt/meeting-actions-to-asana.md)); the [Claude implementation](../providers/claude/meeting-actions-to-asana.md) records the setup a Claude executor would require and remains intended configuration, not deployment. Detection-to-capture latency, repeat-run duplicate avoidance, uncertain-write recovery and reliability across new meetings remain unverified for either runtime. Verify the chosen writer, its invocation model and the affected runtime configuration before calling the workflow enabled; update the existing implementation summary only to reflect observed status.
