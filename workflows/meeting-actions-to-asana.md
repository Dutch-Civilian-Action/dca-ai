...
An owner can reply in this thread with the stable item reference; a bot mention is not required for a registered batch. Receipt in Slack, detection by a monitor, authorisation and verified Asana capture are distinct. State the actual processing mode in the batch: supervised on request, or a verified registered monitor handoff. Sharing a canvas or tagging a bot does not itself register a meeting or trigger processing.

Use the existing [validation monitor](monitor-validation-queue.md) for explicitly authorised, registered batches. Retain their source thread, review-message link, item references, designated writer and pending handoffs in existing lineage. The monitor reads current replies and routes action confirmations to this workflow; it must not treat task consent as validation of organisational facts. Apply the existing [provider contact window and reminder limits](../providers/chatgpt/dca-validation-queue-monitor.md); no second reminder stream. If detection succeeds but dispatch or writing is unavailable, mark the confirmed handoff blocked and preserve the exact missing capability. An unregistered meeting still needs an explicit processing request and authorised registration.
