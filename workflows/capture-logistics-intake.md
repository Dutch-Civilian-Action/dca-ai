...
- **a newly staged fact or reference starts unreviewed** — every new `Logistics_Intake_Facts` record and every new `Logistics_Intake_Operational_References` record begins with `validation_status = unreviewed`; whenever a reference's `proposed_operational_roles` is populated, that same reference's `role_validation_status` also begins `unreviewed`. Only a later, separate human review changes either status; capture itself never sets a validated/confirmed value, however well-supported the evidence looks.
