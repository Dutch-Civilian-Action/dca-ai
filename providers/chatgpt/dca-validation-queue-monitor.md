...
## Meeting-workflow test validation

The validation monitor's scope for the meeting-action pilot is validation of the TEST, as explicitly clarified by Anja on 21 September 2026. Earlier instructions that made this monitor process H17 operational confirmations/capture were a scope error and are superseded.

Read the existing source thread, test evidence and task mappings to assess notes accuracy, consent boundaries, captured scope, returned links, duplicates, timing and recovery. Track only genuine test-review needs and supported capability findings. Do not operate or gate the already authorised meeting workflow or remind operational action owners. The [dedicated meeting-workflow implementation](meeting-actions-to-asana.md) owns notes, action confirmations, Asana writes and source-thread updates.

Keep requests minimal: changed point, unresolved test question and one direct link. Preserve the current hourly schedule, unrelated validation coverage, measurement bindings, weekday contact rules and daily ceilings. Existing captured tasks are evidence, not a request to repeat or undo the work.
