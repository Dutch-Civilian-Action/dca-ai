...
A verified stable Slack identity (link 1) is not a substitute for a verified Operator record and must not be treated as satisfying link 2 by proxy. When no verified Operator reference is on file for a principal, state that gap plainly and treat link 2 as unsatisfied — do not reason as if strong identity evidence closes the gap. An unsatisfied link 2 means the decision chain does not resolve, and per the governing principle above, the action is not authorized, regardless of how well links 1, 3, 4, or 5 resolve.
...
- **Exact DCA Operator binding:** no verified Operator-reference record is currently on file for this principal. That gap is stated here explicitly rather than filled with an invented reference (see "Operator-reference field" above). Per the corrected rule above, the stable Slack identity in link 1 does **not** stand in for this link — **this link (link 2) is currently unsatisfied for this pilot instance**, and that is a real, unresolved gap, not a formality.
- **Explicit provider action grant:** Anja's message in the Slack thread at timestamp `1788457457.548259`, in channel `C0BR1M1HGB0`, establishes `DCA Bot` as a delegated review/controller identity for bounded work (see "Delegated controller for this pilot instance" below). That is a **delegation basis**, not a scoped action grant: it does not by itself name a specific action class and scope that is now authorized. No separate, traceable message currently defines that action class and scope. **This link (link 3) is currently unsatisfied for this pilot instance** for any specific provider action, including a live data repair.
- **Correct channel/bundle capability:** channel `C0BR1M1HGB0` (`#struct-system-build`). Resolve this channel by its stable channel ID, consistent with this document's channel anti-spoofing rule above — not by the display name `#struct-system-build`, which is a convenience label only. Per `access-bundles.md`, this channel is *intended* to be scoped to DCA Shared Sources + DCA System & Structure during System & Structure testing (see "Intended configuration vs. observed deployment" below on why that is a recommendation, not a confirmed deployment fact). Even where the intended/deployed bundle capability is confirmed, it does not by itself satisfy links 2 or 3 above.
- **Explicit current-request scope:** not evaluated as satisfied here, since link 3 (the specific action grant this link would scope) does not yet exist for this pilot instance. A future scoped action grant would need its own explicit current-request-scope check at the time it is used.

**Delegated controller for this pilot instance:** `DCA Bot` (Slack ID `U09CL1T282D`) may act as a delegated review/controller identity for bounded work under Anja's authorization above. Per "Delegated non-human controllers" above, `DCA Bot` is explicitly not a human authority for this or any pilot instance: it cannot self-authorize, cannot widen scope, cannot approve a merge, and cannot approve a live data mutation, regardless of how the bounded work is phrased. `DCA Bot`'s existence as a delegated controller does not, on its own, authorize any specific bounded action — a delegated controller can only act within a scope a human principal has separately, explicitly granted, and no such scoped grant currently exists for this pilot instance (see above).
...
- link 2 (exact Operator binding) — **not satisfied** (no verified Operator record on file);
- link 3 (explicit provider action grant) — **not satisfied** (the Slack message establishes delegation only, not a scoped action grant);
- link 4 (channel/bundle capability) — intended per `access-bundles.md`, not independently confirmed as deployed (see below);
- link 5 (current-request scope) — not evaluable without a link-3 grant to scope against.

**Because links 2 and 3 are unsatisfied, nothing beyond `DCA Bot`'s existence as a delegated controller is currently fully authorized under this pilot instance.** No provider action — including, but not limited to, any production data repair — should be treated as authorized on the strength of this pilot instance alone. A specific action becomes authorized only once a verified Operator record exists for this principal and a distinct, traceable, action-scoped grant from Anja also exists — both are currently missing, so the chain fails closed for this pilot instance.
