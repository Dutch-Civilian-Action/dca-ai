...
| Supporting source/reconciliation evidence, when relevant | Read from `appZ1Fv0YtZPbBbWa` / DCA Evidence & Reconciliation; retain the qualified source reference and uncertainty |

Write relationship intake and canonical results to current Relationships & Workflows only. Relevant Evidence & Reconciliation records may be read under the skill's evidence-access boundary; this does not enable integration writes or move intake there. Do not dual-write to the candidate Shared Identity base `appScO2P8fD8yprCW`, the R01 test base `appTpzRmniNpMv35Q`, or similarly named copies. Existing `Mailchimp_Audiences` and `Mailchimp_Members` tables in Evidence & Reconciliation contained only identifier fields at inspection; their presence does not establish a subscription workflow, current membership or consent. Actual access depends on the configured credential; the workflow's no-write boundary here is not a claim that a two-base PAT is technically read-only in Evidence & Reconciliation.

The pilot needs no schema additions. The existing `Contact_Intake` primary is text and the table lacks a separate visible intake ID/native created/modified fields. Preserve its Airtable record ID and supplied submission time; do not invent an ID field, backfill invented timestamps or migrate a primary during intake. Before any separately authorized schema change, retrieve the live [DCA Airtable Implementation Standard](https://docs.google.com/document/d/17yO7HdChXXvlxekLJjSqiHNgDWmKQm2VstpErtSJ_4c/edit?usp=drivesdk) and follow `skills/dca-airtable-implementation` in the repository.
